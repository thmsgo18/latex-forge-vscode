...
Pick a template (built-in or installed), enter a name, choose the destination folder — the extension runs `latex-forge create` and offers to open the generated project. If the chosen folder already contains LaTeX files, it warns you before nesting projects.
...
A form inside VS Code to edit your LaTeX Forge profile (name, email, university, supervisor…) stored in `~/.latex-forge/profile.toml`. Every project you create afterwards is pre-filled with these values — same file the CLI's `latex-forge profile` uses.
